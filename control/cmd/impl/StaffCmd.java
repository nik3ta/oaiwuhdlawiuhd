...
@CmdInfo(name = "staff", description = "Добавляет игрока в Staf-List")
public class StaffCmd extends Cmd {
    ...
    private void removeStaffName(String name) {
        ...
        if (manager.getStaffNames().contains(name)) {
            ...
            sendMessage(Translated.isRussian() ? TextFormatting.RED + "This player is not on the Staff List!" : TextFormatting.RED + "Такого игрока нет в Staff List!");
        }
    }
    ...
    private void outputList() {
        ...
        if (staffNames.size() > 10) {
            sendMessage(Translated.isRussian()
                    ...
                    : TextFormatting.RED + "Слишком большой стаф лист, загляните в папку с конфигом!");
        } else {
