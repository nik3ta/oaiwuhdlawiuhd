...
public class ProjectionUtils implements IMinecraft {
    public static Vector2f project2f(double x, double y, double z) {
        ...
        if (mc.gameSettings.viewBobbing) {
            ...
            if (renderViewEntity instanceof PlayerEntity) {
                PlayerEntity var10 = (PlayerEntity)renderViewEntity;
            }
        }

        double fov = mc.gameRenderer.getFOVModifier(mc.getRenderManager().info, mc.getRenderPartialTicks(), true);
        return calculateScreenPosition2f(result3f, fov);
    }
    private static Vector2f calculateScreenPosition2f(Vector3f result3f, double fov) {
        float halfHeight = (float)mc.getMainWindow().getScaledHeight() / 2.0F;
        float scaleFactor = halfHeight / (result3f.getZ() * (float)Math.tan(Math.toRadians(fov / (double)2.0F)));
        return result3f.getZ() < 0.0F ? new Vector2f(-result3f.getX() * scaleFactor + (float)mc.getMainWindow().getScaledWidth() / 2.0F, (float)mc.getMainWindow().getScaledHeight() / 2.0F - result3f.getY() * scaleFactor) : null;
    }
